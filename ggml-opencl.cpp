...
void ggml_cl_init(void) {
    ...
    cl_platform = platforms[plat_num];
    char platform_buffer[1024];
    clGetPlatformInfo(cl_platform, CL_PLATFORM_NAME, sizeof(platform_buffer), &platform_buffer, NULL);
    cl_uint num_devices;
    clGetDeviceIDs(cl_platform, CL_DEVICE_TYPE_ALL, 0, NULL, &num_devices);
    cl_device_id* devices = (cl_device_id*)malloc(num_devices*sizeof(cl_device_id));
    clGetDeviceIDs(cl_platform, CL_DEVICE_TYPE_ALL, num_devices, devices, NULL);
    cl_device = devices[dev_num];
    char device_buffer[1024];
    clGetDeviceInfo(cl_device, CL_DEVICE_NAME, sizeof(device_buffer), &device_buffer, NULL);
    size_t ext_str_size;
    clGetDeviceInfo(cl_device, CL_DEVICE_EXTENSIONS, 0, NULL, &ext_str_size);
    char* ext_buffer = (char*) malloc(sizeof(char) * ext_str_size);
    clGetDeviceInfo(cl_device, CL_DEVICE_EXTENSIONS, ext_str_size, ext_buffer, NULL);
    ...
    cl_queue = clCreateCommandQueue(cl_ctx, cl_device, CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &err);
    ...
    program = build_program_from_source(cl_ctx, cl_device, kernel_src.c_str());
...
static cl_mem ggml_cl_pool_malloc(const void* ptr, size_t size, size_t* actual_size, bool* ready, cl_mem_flags flags) {
    ...
    cl_mem mem = clCreateBuffer(cl_ctx, flags, size, NULL, &err);
...
static void ggml_cl_h2d_tensor_2d(cl_command_queue queue, cl_mem dst, size_t offset, const struct ggml_tensor * src, uint64_t i3, uint64_t i2, cl_event* ev) {
    cl_int err;
    ...
    if (nb0 == ts && nb1 == ts*ne0/bs) {
        CL_CHECK(clEnqueueWriteBuffer(queue, dst, CL_FALSE, offset, ne1*nb1, x, 0, NULL, ev), "clEnqueueWriteBuffer");
        return;
    }
    if (nb0 == ts) {
        ...
        CL_CHECK(clEnqueueWriteBufferRect(queue, dst, CL_FALSE, buffer_origin, host_origin, region, ts*ne0/bs, 0, nb1, 0, x, 0, NULL, ev), "clEnqueueWriteBufferRect");
        return;
    }
    for (uint64_t i1 = 0; i1 < ne1; i1++) {
        ...
        CL_CHECK(clEnqueueWriteBufferRect(queue, dst, CL_FALSE, buffer_origin, host_origin, region, 0, 0, nb0, 0, ((const char *)x) + i1*nb0, 0, NULL, ev), "clEnqueueWriteBufferRect");
    }
}
...
static void ggml_cl_mul_mat_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    bool x_ready, y_ready;
    cl_mem d_X = ggml_cl_pool_malloc(src0->data, sizeof(float) * x_ne, &x_size, &x_ready, CL_MEM_READ_ONLY);
    cl_mem d_Y = ggml_cl_pool_malloc(src1->data, sizeof(float) * y_ne, &y_size, &y_ready, CL_MEM_READ_ONLY);
    cl_mem d_D = ggml_cl_pool_malloc(nullptr, sizeof(float) * d_ne, &d_size, nullptr, CL_MEM_WRITE_ONLY);

    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            // copy data to device
            if (!x_ready) {
                ggml_cl_h2d_tensor_2d(cl_queue, d_X, 0, src0, i03, i02, NULL);
            }
            if (!y_ready) {
                ggml_cl_h2d_tensor_2d(cl_queue, d_Y, 0, src1, i03, i02, NULL);
            }
            ...
            CL_CHECK(clEnqueueReadBuffer(cl_queue, d_D, true, 0, sizeof(float) * d_ne, d, 1, &ev_sgemm, NULL), "clEnqueueReadBuffer");
        }
    }
...
static void ggml_cl_mul_mat_f16(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, void * wdata, size_t /* wsize */) {
    ...
    size_t x_size, y_size, d_size;
    bool x_ready, y_ready;
    cl_mem d_X = ggml_cl_pool_malloc(src0->data, sizeof(ggml_fp16_t) * x_ne, &x_size, &x_ready, CL_MEM_READ_ONLY);
    cl_mem d_Y = ggml_cl_pool_malloc(src1->data, sizeof(ggml_fp16_t) * y_ne, &y_size, &y_ready, CL_MEM_READ_ONLY);
    cl_mem d_D = ggml_cl_pool_malloc(nullptr, sizeof(ggml_fp16_t) * d_ne, &d_size, nullptr, CL_MEM_WRITE_ONLY);
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            // copy src1 to device
            if (!y_ready) {
                CL_CHECK(clEnqueueWriteBuffer(cl_queue, d_Y, false, 0, sizeof(ggml_fp16_t) * y_ne, tmp, 0, NULL, NULL), "clEnqueueWriteBuffer");
            }
            ...
            CL_CHECK(clEnqueueReadBuffer(cl_queue, d_D, true, 0, sizeof(ggml_fp16_t) * d_ne, tmp, 1, &ev_sgemm, NULL), "clEnqueueReadBuffer");
...
static void ggml_cl_mul_mat_q_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    bool x_ready, y_ready, q_ready;
    cl_mem d_X;
    if (!mul_mat_vec) {
        d_X = ggml_cl_pool_malloc(src0->data, sizeof(float) * x_ne, &x_size, &x_ready, CL_MEM_READ_WRITE);
    }
    cl_mem d_Y = ggml_cl_pool_malloc(src1->data, sizeof(float) * y_ne, &y_size, &y_ready, CL_MEM_READ_ONLY);
    cl_mem d_D = ggml_cl_pool_malloc(nullptr, sizeof(float) * d_ne, &d_size, nullptr, CL_MEM_WRITE_ONLY);
    cl_mem d_Q;
    if (src0->backend == GGML_BACKEND_CPU) {
        d_Q = ggml_cl_pool_malloc(src0->data, q_sz, &q_size, &q_ready, CL_MEM_READ_ONLY);
    }
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            if (src0->backend == GGML_BACKEND_CPU && !q_ready) {
                ggml_cl_h2d_tensor_2d(cl_queue, d_Q, 0, src0, i03, i02, NULL);
            ...
            if (mul_mat_vec) { // specialized dequantize_mul_mat_vec kernel
                ...
                CL_CHECK(clFinish(cl_queue), "clFinish");
                CL_CHECK(clEnqueueNDRangeKernel(cl_queue, *dmmv, 1, NULL, &global, &local, 0, NULL, &ev_sgemm), "clEnqueueNDRangeKernel");
...
void ggml_cl_transform_tensor(ggml_tensor * tensor) {
    ...
    bool ready;
    *dst = ggml_cl_pool_malloc(tensor->data, q_sz, &q_size, &ready, CL_MEM_READ_ONLY);
    ...
    if (!ready) {
        for (int64_t i3 = 0; i3 < ne3; i3++) {
            for (int64_t i2 = 0; i2 < ne2; i2++) {
                int i = i3*ne2 + i2;
                ggml_cl_h2d_tensor_2d(cl_queue, *dst, i*ne0*ne1, tensor, i3, i2, NULL);
            }
        }
